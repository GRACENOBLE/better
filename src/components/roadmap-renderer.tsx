...
const MainNode = ({ data }: { data: any }) => {
  ...
  return (
    <div
      className={`px-4 sm:px-6 py-3 sm:py-4 shadow-lg rounded-lg bg-yellow-300 border-2 border-gray-800 min-w-[180px] sm:min-w-[200px] font-bold text-center relative cursor-pointer ${
    ...
    >
      ...
      <Handle
        ...
        className="w-3 h-3"
      />
      <div className="text-xs sm:text-sm text-gray-900">{data.label}</div>
    </div>
  );
};
...
const TopicNode = ({ data }: { data: any }) => {
  ...
  return (
    <div
      className={`px-3 sm:px-4 py-2 sm:py-3 shadow-md rounded-lg bg-gray-300 border-2 border-gray-800 min-w-[130px] sm:min-w-[150px] font-semibold text-center relative cursor-pointer ${
        isCompleted ? "line-through opacity-60" : ""
      }`}
      onContextMenu={handleRightClick}
    >
